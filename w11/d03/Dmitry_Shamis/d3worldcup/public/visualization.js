function buildSvg(data) {
  ...
  var minGoalsFor = d3.max(data, function(team) {
    return team.goals_for;
  });
  var maxGoalsAgainst = d3.max(data, function(team) {
    return team.goals_against;
  });
  var minGoalsAgainst = d3.max(data, function(team) {
    return team.goals_against;
  });
...
function project(data) {
  ...
  image.enter()
        ...
        .transition()
          .duration(1000)
          .attr('x', function(d) {return xAxisScale(d.goals_against)})
          .attr('y', function(d) {return yAxisScale(d.goals_for)})
          .attr('dx', -12)
          .attr('dx', -12)
          .attr('opacity', .8)
}
