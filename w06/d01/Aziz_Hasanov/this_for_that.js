...
function randomThis(){
  ...
  var randThis = Math.floor(Math.random()*thisArray.length);
  return thisArray[randThis];
}

function randomThat(){
  // Choose a random entry from the thatArray
  var randThat = Math.floor(Math.random()*thatArray.length);
  return thatArray[randThat];
}
