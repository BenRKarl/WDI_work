...
number = 0
general_assembly[:campuses].each {|k, v| number = number + v.count}

general_assembly[:campuses].map {|k, v| v.length}.inject(&:+)

ri general_assembly[:campuses].values.flatten.count

# How many campuses does GA have in Berlin?
general_assembly[:campuses][:berlin].count
# Print out all GA's campuses addresses
general_assembly[:campuses].each {|x,y| y.each {|x| puts x[:address]}}
