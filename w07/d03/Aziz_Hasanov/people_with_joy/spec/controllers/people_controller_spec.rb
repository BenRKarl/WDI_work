...
describe PeopleController do
  describe 'given a person' do
    # before each test create that person
    before :each do
      @andre = Person.new()
      @andre.name = "Andre"
      @andre.save
    ...
    describe "DELETE destroy" do
      it "destroy person record" do
        expect {delete :destroy, {:id => @andre.id}}.to change(Person, :count).by(-1)
      end
    end # DELETE destroy
  ...
  describe 'POST create' do
    before :each do
      post :create, {:person => {name: "Kathew Lamp", email: "kloo@lampmail.com"}}
    end
    it "response with a redirect" do
